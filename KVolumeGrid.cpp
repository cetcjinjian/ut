...
void KVolumeGrid::drawKVolume()
{
    ...
    for(int i= getStartIndex()+1;i<=getEndIndex();++i)
    {
...
void KVolumeGrid::drawInformation()
{
    ...
    if( pData->kline[getCurrentIndex()+1].openingPrice > pData->kline[getCurrentIndex()+1].closeingPrice )
        pen.setColor(QColor(85,252,252));
    else
        pen.setColor(Qt::red);
    ...
    painter.drawText(rectText,QString::fromLocal8Bit("总手")+str.sprintf(":%.0f",pData->kline[getCurrentIndex()+1].ftotalVolume / 100));


    pen.setColor(Qt::white);
    painter.setPen(pen);
    QRect rectText1(getMarginLeft()+ 80,0,informatonWidth,informatonHeight);
    painter.drawText(rectText1,tr("MAVOL5")+str.sprintf(":%.0f",pData->kline[getCurrentIndex()+1].volumeAverage5 /100));

    pen.setColor(Qt::yellow);
    painter.setPen(pen);
    QRect rectText2(getMarginLeft() +180,0,informatonWidth,informatonHeight);
    painter.drawText(rectText2,tr("MAVOL10")+str.sprintf(":%.0f",pData->kline[getCurrentIndex()+1].volumeAverage10 / 100));
