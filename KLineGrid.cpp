...
void KLineGrid::drawKLine()
{
	...
	for (int i = getStartIndex(); i <= getEndIndex(); ++i)
	{
		...
		else
		{
			...
			p3.setX(getMarginLeft() + xstep *(i - getStartIndex()) + 0.5*lineWidth);
			p3.setY(getWidgetHeight() - (y2 - lowestBid) *yscale - getMarginBottom());
			p4.setX(getMarginLeft() + xstep *(i - getStartIndex()) + 0.5*lineWidth);
	...
	case 5:
		for (int i = getStartIndex(); i <= getEndIndex(); ++i)
		{
			if (pData->kline[i].averageLine5 == 0)
				continue;
			int y = getWidgetHeight() - (pData->kline[i].averageLine5 - lowestBid) *yscale - getMarginBottom();
			if (y < getMarginTop())
				continue;
	...
	case 10:
		for (int i = getStartIndex(); i <= getEndIndex(); ++i)
		{
			if (pData->kline[i].averageLine10 == 0)
				continue;
			int y = getWidgetHeight() - (pData->kline[i].averageLine10 - lowestBid) *yscale - getMarginBottom();
			if (y < getMarginTop())
				continue;
			temp.setX(getMarginLeft() + xstep *(i - getStartIndex()) + 0.5*lineWidth);
	...
	case 20:
		for (int i = getStartIndex(); i <= getEndIndex(); ++i)
		{
			if (pData->kline[i].averageLine20 == 0)
				continue;
			int y = getWidgetHeight() - (pData->kline[i].averageLine20 - lowestBid) *yscale - getMarginBottom();
			if (y < getMarginTop())
				continue;
			temp.setX(getMarginLeft() + xstep *(i - getStartIndex()) + 0.5*lineWidth);
	...
	case 30:
		for (int i = getStartIndex(); i <= getEndIndex(); ++i)
		{
			if (pData->kline[i].averageLine30 == 0)
				continue;
			int y = getWidgetHeight() - (pData->kline[i].averageLine30 - lowestBid) *yscale - getMarginBottom();
			if (y < getMarginTop())
				continue;
			temp.setX(getMarginLeft() + xstep *(i - getStartIndex()) + 0.5*lineWidth);
	...
	case 60:
		for (int i = getStartIndex(); i <= getEndIndex(); ++i)
		{
			if (pData->kline[i].averageLine60 == 0)
				continue;
			int y = getWidgetHeight() - (pData->kline[i].averageLine60 - lowestBid) *yscale - getMarginBottom();
			if (y < getMarginTop())
				continue;
			temp.setX(getMarginLeft() + xstep *(i - getStartIndex()) + 0.5*lineWidth);
			temp.setY(y);
...
void KLineGrid::drawInformation()
{
	...
	painter.drawText(rectText, QString::fromLocal8Bit("江苏索普 MA5:") +
...
void KLineGrid::drawTipsKeyDown(double begin, double end, int types)
{
	...
	QColor openingColor = pData->kline[currentDay].openingPrice > pData->kline[currentDay - 1].openingPrice ?
		QColor("#FF0000") : QColor("#00FF00");

	QColor highestColor = pData->kline[currentDay].highestBid > pData->kline[currentDay - 1].closeingPrice ?
		QColor("#FF0000") : QColor("#00FF00");

	QColor lowestColor = pData->kline[currentDay].lowestBid > pData->kline[currentDay - 1].closeingPrice ?
		QColor("#FF0000") : QColor("#00FF00");


	QColor closeingColor = pData->kline[currentDay].closeingPrice > pData->kline[currentDay].openingPrice ?
		QColor("#FF0000") : QColor("#00FF00");


	QColor amountOfIncreaseColor = pData->kline[currentDay].amountOfIncrease > 0 ?
		QColor("#FF0000") : QColor("#00FF00");


	pdetail->receiveParams(
		pData->kline[currentDay].time, QColor("#FFFFFF"),
		pData->kline[currentDay].closeingPrice, QColor("#FF0000"),
		pData->kline[currentDay].openingPrice, openingColor,
		pData->kline[currentDay].highestBid, highestColor,
		pData->kline[currentDay].lowestBid, lowestColor,
		pData->kline[currentDay].closeingPrice, closeingColor,
		pData->kline[currentDay].amountOfIncrease, amountOfIncreaseColor,
		pData->kline[currentDay].amountOfAmplitude, QColor("#02E2F4"),
		pData->kline[currentDay].ftotalVolume, QColor("#02E2F4"),
		pData->kline[currentDay].totalAmount, QColor("#02E2F4"),
		pData->kline[currentDay].turnoverRate, QColor("#02E2F4")
	);
